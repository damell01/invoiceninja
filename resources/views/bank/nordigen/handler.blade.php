...
<script>
    ...
    const config = {
        ...
        text: "{{ ($account ?? false) && !$account->isPaid() ? config('branding.app_name') : (isset($company) && !is_null($company) ? $company->name : config('branding.app_name')) }} {{ ctrans('texts.nordigen_handler_subtitle', [], $lang ?? 'en') }}",
        // Logo URL that will be shown below the modal form.
        logoUrl: "{{ ($account ?? false) && !$account->isPaid() ? asset(config('branding.logo_light')) : (isset($company) && !is_null($company) ? $company->present()->logo() : asset(config('branding.logo_light'))) }}",
